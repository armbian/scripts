...
function boards
{
        ...
        do
			PARAMETER=""
#			[[ $PARALLEL -gt 1 ]] && PARAMETER="screen -dmSL ${h}$1 "
...
#			echo $PARAMETER
			...
			do
				...
				[[ ${DISPLAY_STAT} == yes ]] && echo -e "Mem: ${FREE_MEM}% Load: $(cat /proc/loadavg | cut -d" " -f1-3) Tasks: $CONCURENT / ${PARALLEL_BUILDS} Time: \x1B[92m$(( CURRENT_TIME - START_TIME ))s\x1B[0m\r"
...
function releases
{
	if [[ -d config/desktop ]]; then
		...
		# add hirsute and bullseye even its not supported
		local releases+=(hirsute bullseye)
	else
